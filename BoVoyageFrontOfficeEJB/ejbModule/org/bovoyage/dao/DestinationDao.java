...
public class DestinationDao {
	...
	public List<DateVoyage> getDateVoyage(Destination destination){
		...
		dateVoyages = destination.getDateVoyages();
		return dateVoyages;
	}
	...
	public List<DestinationTO> getDestinationByRegion(String region){
		List<Destination> dests = em.createNamedQuery("Destination.getDestinationByRegion").setParameter("region", region).getResultList();
		List<DestinationTO> destinations = new ArrayList<>();
		for (Destination d : dests){
			destinations.add(new DestinationTO(d));
		}
